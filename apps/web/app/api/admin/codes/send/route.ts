...
export async function POST(request: Request) {
  ...
  const fromName = process.env.RESEND_FROM_NAME || "UplyFox";
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      ...
      to: [recipient],
      subject: "Your UplyFox access code",
      text: `Your UplyFox access is ready.\n\nAccess code: ${body.code!.trim()}\n\nThis code expires on ${expiresAt} UTC and can be used once. Open UplyFox to continue: ${accessUrl.toString()}\n\nIf you did not request access, you can ignore this email.`,
      html: `<!doctype html><html><body style="margin:0;background:#f6f5fc;color:#252343;font-family:Arial,sans-serif"><div style="max-width:560px;margin:32px auto;padding:0 16px"><div style="border:1px solid #e4e1f2;border-radius:24px;background:#fff;padding:36px;box-shadow:0 18px 55px rgba(55,44,130,.1)"><div style="color:#8b1e3f;font-size:14px;font-weight:700;letter-spacing:.04em">UPLYFOX</div><h1 style="margin:22px 0 10px;font-size:30px;line-height:1.1;color:#202040">Your access is ready.</h1><p style="margin:0;color:#716e87;font-size:15px;line-height:1.6">Your request was approved. Use this one-time code to unlock your UplyFox workspace.</p><div style="margin:26px 0;padding:18px;border:1px solid #d9d4ff;border-radius:14px;background:#f7f5ff;text-align:center"><div style="color:#79758d;font-size:11px;font-weight:700;letter-spacing:.1em;text-transform:uppercase">One-time access code</div><div style="margin-top:10px;color:#4032b9;font-size:24px;font-weight:800;letter-spacing:.12em">${safeCode}</div></div><p style="margin:0 0 22px;color:#716e87;font-size:13px;line-height:1.6">This code expires on ${escapeHtml(expiresAt)} UTC, is tied to ${safeRecipient}, and can only be redeemed once.</p><a href="${safeAccessUrl}" style="display:inline-block;padding:13px 18px;border-radius:10px;background:#5546d9;color:#fff;font-size:14px;font-weight:700;text-decoration:none">Open UplyFox and unlock access</a><p style="margin:28px 0 0;color:#9a97aa;font-size:12px;line-height:1.5">If the button does not work, open the access page and enter the code manually. If you did not request access, you can ignore this email.</p></div><p style="text-align:center;color:#aaa6b8;font-size:11px">UplyFox · Built by Hritik</p></div></body></html>`,
    }),
  });
