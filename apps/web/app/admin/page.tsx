...
function AdminContent() {
  ...
  async function load() {
    try {
      ...
      setRequests(requestRows as AccessRequest[]); setCodes(codeRows as AccessCode[]); setAuthorized(true);
    } catch (error) {
      setAuthorized(false); setNotice(error instanceof Error ? error.message : "Could not load admin console");
    }
  }
  useEffect(() => { void load(); }, []);
  ...
  async function review(requestId: string, decision: "approved" | "rejected") {
    ...
    try { await accessFetch("/api/admin/access-requests", { method: "POST", body: JSON.stringify({ requestId, decision }) }); await load(); setNotice(`Request ${decision}.`); }
    catch (error) { setNotice(error instanceof Error ? error.message : "Could not update request"); }
    finally { setBusy(null); }
  }

  async function createCode(requestId: string) {
    setBusy(requestId); setNotice("");
    try { const result = await accessFetch("/api/admin/codes", { method: "POST", body: JSON.stringify({ requestId }) }); setGeneratedCode(result.code); await load(); }
  ...
  async function revoke(codeId: string) {
    ...
    try { await accessFetch("/api/admin/codes", { method: "POST", body: JSON.stringify({ action: "revoke", codeId }) }); await load(); setNotice("Code revoked."); }
    catch (error) { setNotice(error instanceof Error ? error.message : "Could not revoke code"); }
    finally { setBusy(null); }
  }

  if (!authorized) return <main className="admin-page"><section className="admin-empty"><h1>Admin access required</h1><p>{notice || "This account is not configured as an ApplyPilot administrator."}</p></section></main>;
  const pending = requests.filter((item) => item.status === "pending");
  return <main className="admin-page"><header className="admin-header"><div><span className="admin-kicker">ApplyPilot control room</span><h1>Access requests</h1><p>Approve beta users, issue one-time codes, and keep a clear redemption trail.</p></div><a href="/access" className="admin-back">Back to access</a></header>{generatedCode && <section className="generated-code"><div><span>New code · show this once</span><strong>{generatedCode}</strong><small>Expires in one hour and is bound to the approved email.</small></div><button onClick={() => navigator.clipboard.writeText(generatedCode)}>Copy code</button></section>}{notice && <p className="admin-notice">{notice}</p>}<section className="admin-stats"><div><strong>{pending.length}</strong><span>Pending requests</span></div><div><strong>{requests.filter((item) => item.status === "approved").length}</strong><span>Approved</span></div><div><strong>{codes.filter((item) => item.status === "active").length}</strong><span>Active codes</span></div><div><strong>{codes.filter((item) => item.status === "redeemed").length}</strong><span>Redeemed</span></div></section><section className="admin-panel"><div className="admin-panel-head"><div><span className="admin-kicker">Review queue</span><h2>Requests</h2></div><button className="refresh-button" onClick={load}>Refresh</button></div>{requests.length === 0 ? <p className="admin-muted">No access requests yet.</p> : <div className="request-list">{requests.map((item) => <article className="request-row" key={item.id}><div className="request-main"><strong>{item.email}</strong><span>Requested {new Date(item.created_at).toLocaleString()}</span>{item.message && <p>{item.message}</p>}</div><span className={`status-pill ${item.status}`}>{item.status}</span><div className="request-actions">{item.status === "pending" && <><button className="approve" disabled={busy === item.id} onClick={() => review(item.id, "approved")}>Approve</button><button className="reject" disabled={busy === item.id} onClick={() => review(item.id, "rejected")}>Reject</button></>}{item.status === "approved" && <button className="approve" disabled={busy === item.id} onClick={() => createCode(item.id)}>Generate 1-hour code</button>}</div></article>)}</div>}</section><section className="admin-panel"><div className="admin-panel-head"><div><span className="admin-kicker">Issued invitations</span><h2>One-time codes</h2></div></div>{codes.length === 0 ? <p className="admin-muted">Codes appear here after a request is approved.</p> : <div className="code-list">{codes.map((item) => <article className="code-row" key={item.id}><div><strong>{item.code_prefix}…</strong><span>{item.email}</span></div><span className={`status-pill ${item.status}`}>{item.status}</span><small>{item.status === "active" ? `Expires ${new Date(item.expires_at).toLocaleString()}` : item.redeemed_at ? `Redeemed ${new Date(item.redeemed_at).toLocaleString()}` : ""}</small>{item.status === "active" && <button className="reject" disabled={busy === item.id} onClick={() => revoke(item.id)}>Revoke</button>}</article>)}</div>}</section></main>;
}
