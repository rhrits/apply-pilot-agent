...
function AdminContent() {
  ...
  return <main className="admin-page">
    <header className="admin-header"><div><span className="admin-kicker">ApplyPilot control room</span><h1>Access requests</h1><p>Review requests, issue one-time codes, and email approved users securely.</p></div><a href="/access" className="admin-back">Back to access</a></header>
    {generatedCode && <section className="generated-code"><div><span>New code · show this once</span><strong>{generatedCode}</strong><small>Bound to {generatedEmail}. Expires in one hour.</small><p>Send the styled ApplyPilot email with the same code and a one-click unlock link.</p></div><div className="generated-actions"><button className="send-email-button" onClick={sendCodeEmail} disabled={busy === "send-email"}>{busy === "send-email" ? "Sending…" : "Send code email"}</button><button className="copy-code-button" onClick={() => navigator.clipboard.writeText(generatedCode)}>Copy code</button></div></section>}
