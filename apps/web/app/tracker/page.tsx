...
export default function TrackerPage() {
  ...
  function persist(next: JobApplication[]) { setJobs(next); localStorage.setItem("applypilot-jobs", JSON.stringify(next)); }
  async function syncJob(job: JobApplication) {
    ...
    if (savedJob) await supabase.from("applications").insert({ user_id: userData.user.id, job_id: savedJob.id, status: job.status });
  }
