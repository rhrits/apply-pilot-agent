...
function AccessContent() {
  ...
  if (status.hasAccess) return <main className="access-page"><section className="access-card access-success"><div className="access-brand"><img src="/icons/48.png" width={30} height={30} alt="" /><strong>UplyFox</strong></div><span className="access-badge success">ACCESS ACTIVE</span><h1>Your workspace is ready.</h1><p>Your profile, answer library, tracker, and extension are unlocked.</p><div className="access-actions"><Link className="access-button primary" href={nextPath}>Continue</Link>{status.isAdmin && <Link className="access-button secondary" href="/admin">Admin console</Link>}</div></section></main>;

  const requestStatus = status.request?.status;
  return <main className="access-page"><section className="access-card"><div className="access-brand"><img src="/icons/48.png" width={30} height={30} alt="" /><strong>UplyFox</strong>{status.isAdmin && <Link className="admin-link" href="/admin">Admin console</Link>}</div><span className={`access-badge ${requestStatus === "approved" ? "approved" : requestStatus === "rejected" ? "rejected" : "pending"}`}>{requestStatus === "approved" ? "APPROVED" : requestStatus === "rejected" ? "REQUEST AGAIN" : requestStatus ? "PENDING REVIEW" : "INVITE ONLY"}</span><h1>Get access to UplyFox.</h1><p className="access-lede">UplyFox is in a controlled beta. Request an invite and unlock your workspace with a one-time code.</p><div className="access-steps"><strong>How it works</strong><span>1. Request access.</span><span>2. Wait for approval.</span><span>3. Enter the code from email.</span></div>{requestStatus === "pending" && <div className="access-notice"><strong>Request received.</strong><span>We will review it and email you if approved.</span></div>}{requestStatus === "approved" && <div className="access-notice approved-notice"><strong>Your request was approved.</strong><span>Your code is ready. It expires after one hour and can be used once.</span></div>}{requestStatus === "rejected" && status.request?.reviewer_note && <div className="access-notice"><strong>Request feedback</strong><span>{status.request.reviewer_note}</span></div>}<div className="access-section"><label htmlFor="access-code">Have an access code?</label><div className="access-input-row"><input id="access-code" value={code} onChange={(event) => setCode(event.target.value.toUpperCase())} placeholder="UF-XXXXXX-XXXXXX-XXXXXX-XXXXXX" autoComplete="off" /><button className="access-button primary" onClick={() => void redeemCode()} disabled={busy || code.trim().length < 16}>{busy ? "Checking…" : "Unlock workspace"}</button></div><small>Use the code from your UplyFox email. The email button can fill and apply it automatically.</small></div>{(!requestStatus || requestStatus === "rejected") && <div className="access-section request-section"><label htmlFor="request-message">Request an access code</label><textarea id="request-message" value={requestMessage} onChange={(event) => setRequestMessage(event.target.value)} placeholder="Tell us briefly how you plan to use UplyFox (optional)." rows={3} /><button className="access-button secondary" onClick={() => void requestAccess()} disabled={busy}>{busy ? "Sending…" : requestStatus === "rejected" ? "Submit another request" : "Send request"}</button></div>}{status.onboardingCompletedAt ? <p className="access-foot">Signed in as {status.email}. Your profile is complete.</p> : <p className="access-foot">Complete your profile while your request is reviewed. <Link href="/onboarding">Open onboarding</Link></p>}{message && <p className="access-message">{message}</p>}</section></main>;
}
